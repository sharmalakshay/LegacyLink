...
router.post('/reset_password', async (req, res) => {
    try {
        ...
        if (isMatch) {
            const token = jwt.sign({ id: user.id }, process.env.JWT_SECRET, { expiresIn: '1h' });
            res.cookie('token', token, { httpOnly: true }); // set cookie
            return res.render('redirect_message', { message: 'New password is same as old password. Logging in!', redirect: '' });
        }
        ...
        const token = jwt.sign({ id: user.id }, process.env.JWT_SECRET, { expiresIn: '1h' });
        res.cookie('token', token, { httpOnly: true }); // set cookie
        return res.render('redirect_message', { message: 'Password reset successfully. Logging in!', redirect: '' });
    } catch (error) {
        console.error(error.message);
        res.status(500).json({ message: 'Server Error.' });
    }
